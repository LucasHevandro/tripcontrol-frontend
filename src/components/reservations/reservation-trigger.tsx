...
export function ReservationTrigger({ tripId, variant = "button", label = "Nova reserva" }: ReservationTriggerProps) {
    const [isOpen, setIsOpen] = useState(false);
    const { data: participantsData } = useParticipants(tripId);
    const { user } = useUser();

    const participants = (participantsData?.participants ?? []).map((p) => ({ id: p.id, name: p.name }));
        ...
        <>
            {variant === "button" ? (
                ...
                <button type="button" onClick={() => setIsOpen(true)}
                    ...
                    <p className="text-sm font-medium text-neutral-600 dark:text-neutral-400">Adicionar reserva</p>
                    <p className="text-xs text-neutral-400 dark:text-neutral-500">Hospedagem, passagem, carro ou passeio</p>
                </button>
            )}
            {isOpen && (
                <NewReservationModal tripId={tripId} participants={participants} currentUserId={user?.id ?? ""} onClose={() => setIsOpen(false)} />
            )}
        </>
    );
}