...
import { useDeleteTrip } from "@/hooks/trips/use-trips";
...
export function TripCard({ trip }: TripCardProps) {
    ...
    const canDelete =
        trip.role === "ORGANIZER" && new Date(trip.startDate) > new Date();
        ...
        <div className="relative group">
            ...
            {canDelete && (
                <button
                    ...
                    className="absolute right-2 top-2 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-black/30 text-white opacity-0 backdrop-blur-sm transition-opacity hover:bg-black/50 focus:opacity-100 group-hover:opacity-100 sm:opacity-0 opacity-100"
            ...
            <ConfirmDialog
                ...
                message={`Tem certeza que deseja excluir "${trip.name}"? Todos os dados da viagem serão removidos permanentemente.`}
